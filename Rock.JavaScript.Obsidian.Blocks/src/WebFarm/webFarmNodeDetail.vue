...
<template>
    ...
    <DetailBlock v-if="!blockError"
                 ...
                 @save="onSave">
        <template #view>
            <ViewPanel :modelValue="webFarmNodeViewBag" :options="options" />
        </template>

        <template #edit>
            <EditPanel v-model="webFarmNodeEditBag" :options="options" @propertyChanged="onPropertyChanged" />
        </template>
    </DetailBlock>
</template>
...
<script setup lang="ts">
    ...
    import EditPanel from "./WebFarmNodeDetail/editPanel.partial.vue";
    import ViewPanel from "./WebFarmNodeDetail/viewPanel.partial.vue";
    import { getSecurityGrant, provideSecurityGrant, refreshDetailAttributes, useConfigurationValues, useInvokeBlockAction } from "@Obsidian/Utility/block";
    import { debounce } from "@Obsidian/Utility/util";
    import { NavigationUrlKey } from "./WebFarmNodeDetail/types";
    ...
    const entityTypeGuid = EntityType.WebFarmNode;
    ...
    const validProperties = [
        "attributeValues",
        "currentLeadershipPollingIntervalSeconds",
        "isActive",
        "isCurrentJobRunner",
        "isLeader",
        "lastSeenDateTime",
    ...
    const panelName = computed((): string => {
        return webFarmNodeViewBag.value?.nodeName ?? "";
    });
    ...
    const entityKey = computed((): string => {
        return webFarmNodeViewBag.value?.idKey ?? "";
    });
    ...
    const blockLabels = computed((): PanelAction[] | undefined => {
        ...
        if (webFarmNodeViewBag.value?.isActive === true) {
            labels.push({
    ...
    async function onCancelEdit(): Promise<boolean | string> {
        if (!webFarmNodeEditBag.value?.idKey) {
    ...
    async function onDelete(): Promise<false | string> {
        ...
        if (result.isSuccess && result.data) {
            ...
            errorMessage.value = result.errorMessage ?? "Unknown error while trying to delete web farm node.";
